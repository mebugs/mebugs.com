...
function initHeader() {
  useHead({
    title: `${resData.value.null ? "哎呀呀！没找到！" : resData.value?.main?.title} - 文章标签${runtimeConfig.public.siteName}`,
    meta: [{ hid: "description", name: "description", content: `${resData.value.null ? "哎呀呀！没找到！" : resData.value?.main?.summary} - 文章标签 - ${runtimeConfig.public.description}` }],
  });
}
// 页面挂载后的初始化
onMounted(async () => {
