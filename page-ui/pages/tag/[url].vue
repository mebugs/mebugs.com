    ...
    <div class="r">
      <div class="bcn">
        <div class="totp" v-if="resData.main">
          ...
          <div class="bt">
            <NuxtLink class="bz" to="/tag">返回标签</NuxtLink>
          </div>
        </div>
      </div>
    </div>
...
// 翻页
let query = { by: 7, value: url, page: 1 };
...
function initHeader() {
  useHead({
    title: `${resData.value?.main?.title ? resData.value?.main?.title : "哎呀呀！没找到！"} - 文章标签${runtimeConfig.public.siteName}`,
    meta: [
      { hid: "description", name: "description", content: `${resData.value?.main?.summary ? resData.value?.main?.summary : "哎呀呀！没找到！"} - 文章标签 - ${runtimeConfig.public.description}` },
    ],
  });
}
// 页面挂载后的初始化
