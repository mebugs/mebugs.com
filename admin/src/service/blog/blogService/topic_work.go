...
// 解析数据库错误
func checkTopicDBErr(err error) *baseModel.ResBody {
	errStr := err.Error()
	if strings.Contains(errStr, constant.DBDuplicateErr) {
		if strings.Contains(errStr, "info_uni") {
			// 唯一索引错误
			return baseModel.Fail(constant.TopicUniInfoNG)
		}
	}
	// 默认业务异常
...
// 分页查询对象封装
func topicPageQuery(req *blogModel.TopicPageReq) (query *actuator.Query) {
	// 初始化Page
