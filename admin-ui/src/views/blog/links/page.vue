    ...
    <a-row :gutter="20">
      <a-col :span="8">
        <a-form-item field="title" :label="$t('links.title')">
          <a-input v-model="query.title" :max-length="16" allow-clear show-word-limit :placeholder="$t('links.title.sc')" />
        </a-form-item>
      </a-col>
      <a-col :span="8">
        <a-form-item field="url" :label="$t('links.url')">
          <a-input v-model="query.url" :max-length="16" allow-clear show-word-limit :placeholder="$t('links.url.sc')" />
        </a-form-item>
      </a-col>
      <a-col :span="8"> </a-col>
      <a-col :span="12">
        <a-space>
          <a-tooltip :content="$t('button.add')" :mini="true">
            <a-button
              ...
              type="primary"
              status="danger"
              @click="pop.open('add', 0, $t('links.add'), $t('links.add.sub'), {}, search)">
              <template #icon>
                <icon-plus />
              </template>
            </a-button>
  ...
  <a-spin :loading="load">
    <a-row :gutter="20">
      ...
      <a-col :span="12" v-for="record in list" :key="record.id">
        <div class="cardlist linksList">
          <div class="upIcon">
            <img :src="record.sourceShow" />
            <a-tag class="fTag" :color="record.status == '0' ? 'green' : record.status == '2' ? 'red' : 'orange'">
              {{ record.status == '0' ? '已通过' : record.status == '2' ? '已拒绝' : '待审核' }}
            </a-tag>
          </div>
          <div class="linksInf">
            <p class="linksT">{{ record.title }} / {{ record.url }}</p>
            <p class="linksS">{{ record.summary }}</p>
          </div>
          <a-space class="linksBtn">
            <a-tooltip :content="$t('button.get')" :mini="true">
              <a-button v-permission="''" type="text" @click="pop.open('get', record.id, $t('links.get'), record.title, {}, search)">
                <template #icon> <icon-eye /> </template>
              </a-button>
            </a-tooltip>
            <a-tooltip :content="$t('button.edit')" :mini="true">
              <a-button v-permission="''" type="text" @click="pop.open('edit', record.id, $t('links.edit'), record.title, {}, search)">
...
import { linksPage, linksDel } from '@/api/blog/links'
// 入参读取
...
async function pageQuery() {
  ...
  setLoad(true)
  try {
    const res = await linksPage({ ...query.value, ...page })
