    ...
    <a-row :gutter="20">
      <a-col :span="8">
        <a-form-item field="title" :label="$t('topic.title')">
          <a-input v-model="query.title" :max-length="16" allow-clear show-word-limit :placeholder="$t('topic.title.sc')" />
        </a-form-item>
      </a-col>
      <a-col :span="8">
        <a-form-item field="url" :label="$t('topic.url')">
          <a-input v-model="query.url" :max-length="16" allow-clear show-word-limit :placeholder="$t('topic.url.sc')" />
        </a-form-item>
      </a-col>
      <a-col :span="8"> </a-col>
      <a-col :span="12">
        <a-space>
          <a-tooltip :content="$t('button.add')" :mini="true">
            <a-button
              ...
              type="primary"
              status="danger"
              @click="pop.open('add', 0, $t('topic.add'), $t('topic.add.sub'), {}, search)">
              <template #icon>
                <icon-plus />
              </template>
            </a-button>
    ...
    <a-row :gutter="20">
      ...
      <a-col :span="6" v-for="record in list" :key="record.id">
        <div class="cardlist">
          ...
          <p class="twoLine">{{ record.summary }}</p>
          <a-space>
            <a-tooltip :content="$t('button.get')" :mini="true">
              <a-button v-permission="''" type="text" @click="pop.open('get', record.id, $t('topic.get'), record.title, {}, search)">
                <template #icon> <icon-eye /> </template>
              </a-button>
            </a-tooltip>
            <a-tooltip :content="$t('button.edit')" :mini="true">
              <a-button v-permission="''" type="text" @click="pop.open('edit', record.id, $t('topic.edit'), record.title, {}, search)">
...
import { topicPage, topicDel } from '@/api/blog/topic'
// 入参读取
...
async function pageQuery() {
  ...
  setLoad(true)
  try {
    const res = await topicPage({ ...query.value, ...page })
