<template>
  <a-form size="large" :model="query" label-align="left" layout="vertical">
    <a-row :gutter="20">
      <a-col :span="8">
        <a-form-item field="title" :label="$t('tag.title')">
          <a-input v-model="query.title" :max-length="16" allow-clear show-word-limit :placeholder="$t('tag.title.sc')" />
        </a-form-item>
      </a-col>
      <a-col :span="8">
        <a-form-item field="url" :label="$t('tag.url')">
          <a-input v-model="query.url" :max-length="16" allow-clear show-word-limit :placeholder="$t('tag.url.sc')" />
        </a-form-item>
      </a-col>
      <a-col :span="8"> </a-col>
      <a-col :span="12">
        <a-space>
          <a-tooltip :content="$t('button.add')" :mini="true">
            <a-button
              ...
              type="primary"
              status="danger"
              @click="pop.open('add', 0, $t('tag.add'), $t('tag.add.sub'), {}, search)">
              <template #icon>
                <icon-plus />
              </template>
            </a-button>
        ...
        <div class="cardlist">
          ...
          <a-space>
            <a-tooltip :content="$t('button.get')" :mini="true">
              <a-button v-permission="''" type="text" @click="pop.open('get', record.id, $t('tag.get'), record.title, {}, search)">
                <template #icon> <icon-eye /> </template>
              </a-button>
            </a-tooltip>
            <a-tooltip :content="$t('button.edit')" :mini="true">
              <a-button v-permission="''" type="text" @click="pop.open('edit', record.id, $t('tag.edit'), record.title, {}, search)">
...
import { tagPage, tagDel, tagList } from '@/api/blog/tag'
// 入参读取
...
async function pageQuery() {
  ...
  setLoad(true)
  try {
    const res = await tagPage({ ...query.value, ...page })
