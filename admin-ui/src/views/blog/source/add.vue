    ...
    <a-row :gutter="20">
      ...
      <a-col :span="12">
        <a-form-item field="img" :label="$t('source.img')" :rules="[{ required: true, message: $t('rule.required') }]">
          <label class="upImg" for="upMainImgSourceAdd">
...
import { sourceInit, sourceAdd } from '@/api/blog/source'
...
const resetImg = () => {
  ...
  formData.value.img = ''
  const fileInput = document.getElementById('upMainImgSourceAdd') as HTMLInputElement
  if (fileInput) {
    fileInput.value = ''
  }
}
